...
class _BNode(object):
	...
	def lateral(self, parent, parent_index, dest, dest_index):
		if parent_index > dest_index:
			...
			dest.contents.wstaw(0, parent.contents[parent_index])
			parent.contents[parent_index] = self.contents.pop()
			if self.children:
				dest.children.wstaw(0, self.children.pop())
	...
	def shrink(self, ancestors):
		...
		parent.contents.wstaw(parent_index, push)
		parent.children.wstaw(parent_index + 1, sibling)
...
class _BPlusLeaf(_BNode):
	...
	def lateral(self, parent, parent_index, dest, dest_index):
		if parent_index > dest_index:
			...
			dest.dane.append(self.data.pop(0))
			parent.contents[dest_index] = self.contents[0]
		else:
			dest.contents.wstaw(0, self.contents.pop())
			dest.dane.wstaw(0, self.data.pop())
